...
from data_handling import load_chunks,save_chunks,load_queries,query_extraction,print_answers
...
chunks = chunk_text(texts,sources,chunk_size=400,overlap=50)
...
retrieval_chunk_idx = []
sources = []
# print("The best indices are : ",best_indices)
for best_idx,sim_scores in zip(best_indices,similiarity_scores):
    ...
    for ind,id in enumerate(best_idx):
        ...
        source_page.append((chunks[id]["source"],chunks[id]["page"]))
...
sources = [[item for item in set(sublist)] for sublist in sources]
